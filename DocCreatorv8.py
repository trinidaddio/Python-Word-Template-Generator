...
def generate_ai_sectional_content(intended_use, document_title, section_header, additional_prompts, model_name):
    ...
    try:
        ...
        if not api_key:
            return "Error: No Gemini API key found in environment variables."
            ...
            try:
                ...
                for col_header in df.columns:
                    if heading_pattern.match(col_header):
                        ...
                        # If in selective mode, skip this entire section if it's not checked
                        if selective_mode and not selection_data.get(filename, {}).get(heading_content, False):
                            continue
                ...
                usable_width = page_width - section.PageSetup.LeftMargin - section.PageSetup.RightMargin
                ...
                progress_queue.put(("log", f"[ERROR] Could not read the Excel file: {e}\n"))
            except pythoncom.com_error as e:
                progress_queue.put(("log", f"[ERROR] A Word/COM error occurred: {e}\n"))
            except Exception as e:
                import traceback
                error_details = traceback.format_exc()
                progress_queue.put(("log", f"[FAILED] An unexpected error occurred with {filename}: {e}\n{error_details}\n"))
...
class DocCreatorApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(step1_inner, text="Browse...", command=self.select_template_file, width=120, height=40, fg_color=self.colors["gray-600"], hover_color=self.colors["gray-700"], font=self.fonts["button"])
        .grid(row=0, column=2)
        ...
        ctk.CTkButton(step2_inner, text="Browse...", command=self.select_excel_file, width=120, height=40, fg_color=self.colors["gray-600"], hover_color=self.colors["gray-700"], font=self.fonts["button"])
        .grid(row=0, column=2)
        ...
        ctk.CTkLabel(mode_header_frame, text="Step 3: AI Content Generation", font=self.fonts["card_title"], text_color=self.colors["gray-300"])
        ...
        ctk.CTkLabel(model_frame, text="AI Model", font=self.fonts["button"], text_color=self.colors["gray-400"])
        ...
        ctk.CTkLabel(ai_card, text="Intended Use Statement", font=self.fonts["button"], text_color=self.colors["gray-400"])
        ...
        ctk.CTkCheckBox(additional_prompts_frame, text="Target Audience", variable=self.prompt_cb_var_1, font=self.fonts["button"])
        .grid(row=0, column=0, sticky="w")
        ...
        ctk.CTkCheckBox(additional_prompts_frame, text="Tone of Voice", variable=self.prompt_cb_var_2, font=self.fonts["button"])
        ...
        ctk.CTkCheckBox(additional_prompts_frame, text="Include Keywords", variable=self.prompt_cb_var_3, font=self.fonts["button"])
        ...
        ctk.CTkButton(step4_inner, text="Browse...", command=self.select_output_folder, width=120, height=40, fg_color=self.colors["gray-600"], hover_color=self.colors["gray-700"], font=self.fonts["button"])
        ...
        # Right Frame
        ctk.CTkLabel(right_frame, text="Status Log / Section Selector", font=self.fonts["card_title"], text_color=self.colors["gray-300"])
        ...
        ctk.CTkLabel(self.filter_frame, text="Filter:", font=self.fonts["button"])
        .pack(side="left", padx=(0, 5))
        ctk.CTkCheckBox(self.filter_frame, text="Level 1 (Hn)", variable=self.filter_var1, font=self.fonts["button"], command=self._apply_tree_filter)
        .pack(side="left", padx=5)
        ctk.CTkCheckBox(self.filter_frame, text="Level 2 (Hn-n)", variable=self.filter_var2, font=self.fonts["button"], command=self._apply_tree_filter)
        .pack(side="left", padx=5)
        ctk.CTkCheckBox(self.filter_frame, text="Level 3+ (Hn-..)", variable=self.filter_var3, font=self.fonts["button"], command=self._apply_tree_filter)
        .pack(side="left", padx=5)
    ...
    def _apply_tree_filter(self):
        ...
        parent_filenames = {pid: self.selection_tree.item(pid, "text").strip() for pid in self.selection_tree.get_children()}

        children_by_filename = {}
        for item_id, (filename, _, level) in self.tree_item_map.items():
            if filename not in children_by_filename:
                children_by_filename[filename] = []
            children_by_filename[filename].append((item_id, level))

        for parent_id, parent_filename in parent_filenames.items():
            for child in self.selection_tree.get_children(parent_id):
                self.selection_tree.detach(child)

            if parent_filename in children_by_filename:
                for item_id, level in children_by_filename[parent_filename]:
                    should_show = (level == 1 and show_level1) or \
                                  (level == 2 and show_level2) or \
                                  (level >= 3 and show_level3)
                    if should_show:
    ...
    def start_generation(self):
        ...
        self.progress_bar.grid(row=7, column=0, sticky="ew", pady=(4, 6))
    ...
    def open_excel_file(self):
        if self.excel_path and self.excel_path.is_file():
            ...
            os.startfile(os.path.realpath(self.excel_path))
    ...
    def open_output_folder(self):
        """Opens the selected output folder in the default file explorer."""
        if self.output_folder and self.output_folder.is_dir():
            os.startfile(os.path.realpath(self.output_folder))

    def _find_normal_template(self):
        """Finds the default Word Normal.dotm template."""
        try:
            ...
            # Using after() to schedule the log_message call on the main thread
            self.after(0, self.log_message, f"Error finding Normal.dotm: {e}")
        return None
if __name__ == "__main__":
    # --- FIX: Handle Gemini API Key ---
    if not os.environ.get("GEMINI_API_KEY"):
        messagebox.showwarning("API Key Not Found", "The GEMINI_API_KEY environment variable is not set. AI features will be disabled.")
    app = DocCreatorApp()
    app.mainloop()
